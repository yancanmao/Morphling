...
//				if (jobRescalePartitionAssignment == null) {
					subKeyedStates = reAssignSubKeyedStates(
						operatorState,
...
//				}
	...
	private Tuple2<List<KeyedStateHandle>, List<KeyedStateHandle>> reAssignSubKeyedStates(
		...
		if (newParallelism == oldParallelism && !isForceRescale) {
...
//			int subTaskIndex) {
//
//		List<KeyedStateHandle> subManagedKeyedState = new ArrayList<>();
//		List<KeyedStateHandle> subRawKeyedState = new ArrayList<>();
//
//		Map<Integer, List<Integer>> partitionAssignment = jobRescalePartitionAssignment.getPartitionAssignment();
//		Map<Integer, Integer> partitionKeyGroupMapping = jobRescalePartitionAssignment.getPartitionKeyGroupMapping();
//
//		int keyGroupRangeStart = 0;
//		for (int i = 0; i < subTaskIndex; i++) {
//			keyGroupRangeStart += partitionAssignment.get(i).size();
//		}
//
//		for (int i = 0; i < partitionAssignment.get(subTaskIndex).size(); i++) {
//			int assignedKeyGroup = partitionAssignment.get(subTaskIndex).get(i);
//			int mappedAssignedKeyGroup = partitionKeyGroupMapping.get(assignedKeyGroup);
//
//			for (int j = 0; j < operatorState.getParallelism(); j++) {
//				if (operatorState.getState(j) != null) {
//
//					for (KeyedStateHandle keyedStateHandle : operatorState.getState(j).getManagedKeyedState()) {
//						if (keyedStateHandle != null) {
//							KeyedStateHandle intersectedKeyedStateHandle =
//								keyedStateHandle.getIntersection(new KeyGroupRange(mappedAssignedKeyGroup, mappedAssignedKeyGroup));
//
//							KeyedStateHandle assignedKeyedStateHandle = assignNewKeyGroupRange(
//								mappedAssignedKeyGroup,
//								new KeyGroupRange(keyGroupRangeStart + i, keyGroupRangeStart + i),
//								intersectedKeyedStateHandle);
//
//							if (assignedKeyedStateHandle != null) {
//								subManagedKeyedState.add(assignedKeyedStateHandle);
//							}
//						}
//					}
//
//					for (KeyedStateHandle keyedStateHandle : operatorState.getState(j).getRawKeyedState()) {
//						if (keyedStateHandle != null) {
//							KeyedStateHandle intersectedKeyedStateHandle =
//								keyedStateHandle.getIntersection(new KeyGroupRange(mappedAssignedKeyGroup, mappedAssignedKeyGroup));
//
...
//
//							if (assignedKeyedStateHandle != null) {
//								subRawKeyedState.add(assignedKeyedStateHandle);
//							}
//						}
//					}
//				}
...
//	private static KeyedStateHandle assignNewKeyGroupRange(
//			int assignedKeyGroup,
...
//			KeyGroupsStateHandle keyGroupsStateHandle = (KeyGroupsStateHandle) keyedStateHandle;
//
//			if (keyGroupsStateHandle.getKeyGroupRange().equals(KeyGroupRange.EMPTY_KEY_GROUP_RANGE)) {
//				return null;
//			}
//
//			long offset = keyGroupsStateHandle.getOffsetForKeyGroup(assignedKeyGroup);
//			KeyGroupRangeOffsets keyGroupRangeOffsets = new KeyGroupRangeOffsets(keyGroupRange, new long[]{offset});
//
//			return new KeyGroupsStateHandle(keyGroupRangeOffsets, keyGroupsStateHandle.getDelegateStateHandle());
//		} else {
//			throw new RuntimeException("++++++ unsupported KeyedStateHandle for state migration");
//		}
//	}
